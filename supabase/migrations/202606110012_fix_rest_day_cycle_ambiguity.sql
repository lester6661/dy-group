...
as $$
declare
  requested_cycle_year integer := save_my_rest_days.cycle_year;
  requested_cycle_month integer := save_my_rest_days.cycle_month;
  cycle_start date := public.get_rest_cycle_start(requested_cycle_year, requested_cycle_month);
  cycle_end date := public.get_rest_cycle_end(requested_cycle_year, requested_cycle_month);
...
begin
  ...
  from unnest(save_my_rest_days.rest_dates) as rest_date_item;
  ...
  select e.* into current_employee
  from public.employees e
  where e.deleted_at is null
    and (
      e.profile_id = current_profile.id
      or lower(e.email) = lower(current_profile.email)
    )
  order by
    case when e.profile_id = current_profile.id then 0 else 1 end,
    e.created_at
  limit 1;
  ...
  if current_employee.profile_id is null then
    ...
    set profile_id = current_profile.id,
        updated_at = now()
    where e.id = current_employee.id;

    current_employee.profile_id := current_profile.id;
  end if;

  if requested_cycle_year <> extract(year from available_cycle)::integer
     or requested_cycle_month <> extract(month from available_cycle)::integer then
  ...
  set status = 'cancelled',
      updated_at = now()
  where rd.employee_id = current_employee.id
    and rd.cycle_year = requested_cycle_year
    and rd.cycle_month = requested_cycle_month
  ...
  foreach rest_date_value in array normalized_rest_dates loop
    ...
    values (
      ...
      rest_date_value,
      requested_cycle_year,
      requested_cycle_month,
      'manual',
      'confirmed'
    )
