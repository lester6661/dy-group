-- Registration approval flow V2. Split from enum migration so the new status value is committed first.

drop function if exists public.approve_registration_with_employee(uuid, uuid, uuid, uuid, date, time, time);
drop function if exists public.approve_registration_with_employee(uuid, uuid, uuid, public.employee_status, date, time, time, boolean, numeric);
...
begin
  ...
  confirm_date := (approve_registration_with_employee.hire_date + interval '3 months')::date;
  ...
  if employee_id is null then
    ...
    set profile_id = target_profile.id,
        ...
        probation_confirm_date = confirm_date,
...
create or replace function public.auto_confirm_probation_employees()
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  where status = 'probation'
    and probation_confirm_date is not null
    and probation_confirm_date <= current_date
